            ...
            <div class="container">
                <div class="row pt-20 pb-20">
                    <div class="col-md-3 col-xs-6">
                        <a href="{{ route('blogHome') }}"><img class="logo"
                                src="{{ asset('blog') }}/imgs/theme/logo.png" alt=""></a>
                    </div>
                    ...
                    <div class="col-md-9 col-xs-6 text-end header-top-right ">
                            ...
                            <ul class="list-inline nav-topbar d-none d-md-inline">
                                ...
                                <li>
                                    @if (Auth::guard('author')->user()->image == null)
                                        <img style="border-radius: 50% !important;" class="avatar" width="40"
                                            src="{{ Avatar::create(Auth::guard('author')->user()->name)->toBase64() }}" />
                                    @else
                                        <img style="border-radius: 50% !important;" width="40" src="{{ asset('uploads/user/profile/'.Auth::guard('author')->user()->image) }}" alt="User">
                                    @endif

                                    {{-- <a style="color: white!important; font-size: 18px; padding: .5px 5px" class="" href="#"><b>{{ Auth::guard('author')->user()->name }}</b></a> --}}

                                    <ul class="sub-menu">
                                            <h6 style="padding: 5px; color: white; text-align: center; text-decoration: underline overline; background-color: blue"><strong>{{ Auth::guard('author')->user()->name }}</strong></h6>
                                        <li><a style="font-size: 12px!important; border-right: 3px solid blue; padding: 0;"
                                                href="{{ route('author', Auth::guard('author')->user()->id) }}"><b>-
                                                    Profile</b></a></li>
                                        @if (Auth::guard('author')->user()->authorStatus == null)
                                            @if (App\Models\author_request::where('email', Auth::guard('author')->user()->email)->exists())
                                                <li><a
                                                        style="color: green; font-size: 12px!important; border-right: 2px solid red; padding: 0;"><strong>-
                                                            Approval pending</strong></a></li>
                                            @else
                                                <li><a style="font-size: 12px!important; border-right: 3px solid yellow; padding: 0;"
                                                        href="{{ route('requestForAuthor') }}"><b>- Request for
                                                            Author</b></a></li>
                                            @endif
                                        @else
                                            <li><a target="_blank"
                                                    style="font-size: 12px!important; border-right: 3px solid rgb(0, 255, 255); padding: 0;"
                                                    href="{{ route('blogUserDashboard') }}">- Dashborad</a></li>
                                        @endif
                                        <li><a style="color: red; font-size: 12px!important; border-right: 3px solid rgb(30, 255, 0); padding: 0;"
                                                href="{{ route('blogSignOut') }}"><b>- Sign out</b></a></li>
                        ...
                        @else
                            <a class="btn btn-radius bg-primary text-white ml-15 font-small box-shadow"
                                href="{{ route('blogSignIn') }}" target="_self">Sign in</a>
                        @endauth
        ...
        <div class="header-sticky">
            <div class="container align-self-center position-relative">
                ...
                <div class="main-nav d-none d-lg-block float-start">
                    <nav>
                        <!--Desktop menu-->
                        <ul class="main-menu d-none d-lg-inline font-small2">
                            <li>
                                <a href="{{ route('blogHome') }}"> <i class="elegant-icon icon_house_alt mr-5"></i>
                                    Home</a>
                            </li>

                            <li class="current-item has-mega-menu">
                                <a href="category-list.html">Menu</a>
                                <ul class="mega-menu">
                                    <li class="sub-mega-menu sub-mega-menu-width-22">
                                        ...
                                        <ul>
                                            ...
                                            @foreach ($categories as $category)
                                                <li><a href="{{ route('categoryPosts', $category->id) }}">{{ $category->categoryName }}
                                                        ({{ App\Models\Post::where('categoryId', $category->id)->where("status", 1)->count() }})
                                                    </a>
                                                </li>
                                            @endforeach
                                        </ul>
                            ...
                            @php
                                ...
                                $EntertainmentId = 14;
                            @endphp
                            <li><a href="{{ route('categoryPosts', $BussinessId) }}">Bussiness</a> </li>
                            <li> <a href="{{ route('categoryPosts', $EntertainmentId) }}">Entertainment</a> </li>
                ...
                @foreach ($categories as $category)
                    <div class="col-lg-4 col-md-6">
                        <div class="sidebar-widget widget-latest-posts mb-30 wow fadeInUp animated">
                            <div class="widget-header-2 position-relative mb-30">
                                <h5 class="mt-5 mb-30">{{ $category->categoryName }}</h5>
                            </div>
                            <div class="post-block-list post-module-1">
                                <ul class="list-post">
                                    @php
                                      $categoryposts = App\Models\Post::where("categoryId", $category->id)->where("status", 1)->take(3)->get();
                                    @endphp
                                    ...
                                    <li class="mb-30">
                                        <div class="d-flex hover-up-2 transition-normal">
                                            <div
                                                class="post-thumb post-thumb-80 d-flex mr-15 border-radius-5 img-hover-scale overflow-hidden">
                                                <a class="color-white" href="{{ route('postDetails', $categorypost->slug) }}">
                                                    <img src="{{ asset("uploads/post/thumbnail/".$categorypost->thumbnail) }}"
                                                        alt="">
                                                </a>
                                            </div>
                                            <div class="post-content media-body">
                                                <h6 class="post-title mb-15 text-limit-2-row font-medium"><a
                                                        href="{{ route('postDetails', $categorypost->slug) }}">{{ $categorypost->title }}</a></h6>
                                                <div class="entry-meta meta-1 float-start font-x-small text-uppercase">
                                                    <span class="post-on">{{ $categorypost->created_at->diffForHumans() }}</span>
                                                    <span class="post-by has-dot">
                                                        @if (App\Models\popularPost::where("postId", $categorypost->id)->exists())
                                                        {{ App\Models\popularPost::where("postId", $categorypost->id)->first()->totalRead }}
                                                        @else
                                                        0
                                                        @endif
                                                         views</span>
                                ...
                                <li class="mb-30">
                                    <div class="d-flex hover-up-2 transition-normal">
                                        <div
                                            class="post-thumb post-thumb-80 d-flex mr-15 border-radius-5 img-hover-scale overflow-hidden">
                                            <a class="color-white" href="single.html">
                                                <img src="{{ asset('blog') }}/imgs/news/thumb-5.jpg" alt="">
                                            </a>
                                        </div>
                                        <div class="post-content media-body">
                                            <h6 class="post-title mb-15 text-limit-2-row font-medium"><a
                                ...
                                <li class="mb-30">
                                    <div class="d-flex hover-up-2 transition-normal">
                                        <div
                                            class="post-thumb post-thumb-80 d-flex mr-15 border-radius-5 img-hover-scale overflow-hidden">
                                            <a class="color-white" href="single.html">
                                                <img src="{{ asset('blog') }}/imgs/news/thumb-6.jpg" alt="">
                                            </a>
                                        </div>
                                        <div class="post-content media-body">
                                            <h6 class="post-title mb-15 text-limit-2-row font-medium"><a
                                ...
                                <li class="mb-30">
                                    <div class="d-flex hover-up-2 transition-normal">
                                        <div
                                            class="post-thumb post-thumb-80 d-flex mr-15 border-radius-5 img-hover-scale overflow-hidden">
                                            <a class="color-white" href="single.html">
                                                <img src="{{ asset('blog') }}/imgs/news/thumb-7.jpg" alt="">
                                            </a>
                                        </div>
                                        <div class="post-content media-body">
                                            <h6 class="post-title mb-15 text-limit-2-row font-medium"><a
                                                    href="single.html">Which TBA Preset Pack is Right For You?</a>
                                            </h6>
                                            <div class="entry-meta meta-1 float-start font-x-small text-uppercase">
                                ...
                                <li class="mb-30">
                                    <div class="d-flex hover-up-2 transition-normal">
                                        <div
                                            class="post-thumb post-thumb-80 d-flex mr-15 border-radius-5 img-hover-scale overflow-hidden">
                                            <a class="color-white" href="single.html">
                                                <img src="{{ asset('blog') }}/imgs/news/thumb-8.jpg" alt="">
                                            </a>
                                        </div>
                                        <div class="post-content media-body">
                                            <h6 class="post-title mb-15 text-limit-2-row font-medium"><a
                                ...
                                <li class="mb-30">
                                    <div class="d-flex hover-up-2 transition-normal">
                                        <div
                                            class="post-thumb post-thumb-80 d-flex mr-15 border-radius-5 img-hover-scale overflow-hidden">
                                            <a class="color-white" href="single.html">
                                                <img src="{{ asset('blog') }}/imgs/news/thumb-9.jpg" alt="">
                                            </a>
                                        </div>
                                        <div class="post-content media-body">
                                            <h6 class="post-title mb-15 text-limit-2-row font-medium"><a
                                                    href="single.html">Beginner’s Guide to Travel Drone
                                                    Photography</a></h6>
                                            <div class="entry-meta meta-1 float-start font-x-small text-uppercase">
                ...
                <div class="carausel-3-columns">
                    ...
                    @foreach ($categories as $category)
                        <div class="carausel-3-columns-item d-flex bg-grey has-border p-25 hover-up-2 transition-normal border-radius-5"
                            ...
                            <div
                                class="post-thumb post-thumb-64 d-flex mr-15 border-radius-5 img-hover-scale overflow-hidden">
                                <a class="color-white" href="{{ route('categoryPosts', $category->id) }}">
                                    <img src="{{ asset('uploads/categories/' . $category->categoryImage) }}"
                                        alt="{{ $category->categoryName }}">
                                </a>
                            </div>
                            <div class="post-content media-body">
                                <h6> <a href="{{ route('categoryPosts', $category->id) }}">{{ $category->categoryName }}
                                        ({{ App\Models\Post::where('categoryId', $category->id)->count() }})
                                    </a> </h6>
                                <p class="text-muted font-small" style="text-align: justify !important">
                                    {{ Str::substr($category->details, 0, 100) }}... </p>
                            </div>
                        </div>
                    @endforeach
                    {{-- <div
                    ...
                    <div class="sidebar-widget widget_newsletter wow fadeInUp animated mb-30" data-wow-delay="0.3s">
                        ...
                        <div id="subscribe" class="newsletter">
                            <p class="font-medium">Subscribe to our newsletter and get our newest updates right on
                                your inbox.</p>
                                @if (session("exists"))
                                <strong class="alert alert-warning">{{ session("exists") }}</strong>
                                @endif
                                @if (session("done"))
                                <strong class="alert alert-success">{{ session("done") }}</strong>
                                @endif
                            <form class="input-group form-subcriber mt-30 d-flex" action="{{ route("subscribe") }}" method= "POST">
                                @csrf
                                <input type="text" class="form-control bg-white font-small"
